  ...
  return (
    <nav className="w-full md:px-10 md:py-6 flex justify-between items-center">
      ...
      <div className="md:hidden">
        <button className="absolute right-6 z-50" onClick={() => setIsOpen(!isOpen)}>
          <Menu size={24} />
        </button>
      </div>
      ...
      {isOpen && (
        ...
        >
          ...
          <button className="px-4 py-2 rounded-full bg-black text-white hover:bg-gray-800 transition-colors" onClick={() => setIsOpen(false)}>
            Contact Us
          </button>
        </motion.div>
      )}
